...
function Sprite(scene, imageFile, width, height) {
    ...
    this.checkBounds = function() {
            ...
            camX = 0;
            camY = 0;
            if (this.camera) { camX = this.camera.cameraOffsetX;
    ...
    this.generateAnimationCycles = function(slicingFlag, framesArray) {
        ...
        if (typeof framesArray == "number" || typeof slicingFlag == "undefined") {
            if (slicingFlag == SINGLE_COLUMN) { numCycles = (iHeight / cHeight) / framesArray; } else if (typeof slicingFlag == "undefined") { numCycles = (iHeight / cHeight);
                framesArray = iWidth / cWidth; } else { numCycles = (iWidth / cWidth) / framesArray; }
